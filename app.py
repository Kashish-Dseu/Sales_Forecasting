...
with nav_cols[0]:
    home_btn = st.button("🏠 Home", width='stretch')
with nav_cols[1]:
    overview_btn = st.button("📊 Overview", width='stretch')
with nav_cols[2]:
    forecast_btn = st.button("📈 Forecast", width='stretch')
with nav_cols[3]:
    anomaly_btn = st.button("🚨 Anomalies", width='stretch')
with nav_cols[4]:
    demand_btn = st.button("🎯 Segments", width='stretch')
...
# ----------------------------------------------------
    ...
    # --- Segfault-Proof Multi-Model Comparison Infrastructure ---
    @st.cache_data(show_spinner=False)
    def evaluate_all_forecasting_models(train_series, test_series):
        results = {}
        predictions = {}
        
        # Build features for classical ML formulations using Lag and Seasonal-Lag values
        df_ml = ts.copy()
        for lag in [1, 2, 3, 12]:
            df_ml[f'lag_{lag}'] = df_ml['Sales'].shift(lag)
        df_ml = df_ml.fillna(method='bfill').fillna(0)
        ...
        # 1. SARIMA Model Formulation via structural seasonal lag components
        lr_sarima = LinearRegression()
        X_train_s = X_train[[col for col in X_train.columns if 'lag_' in col]]
        X_test_s = X_test[[col for col in X_test.columns if 'lag_' in col]]
        lr_sarima.fit(X_train_s, y_train)
        sarima_pred = pd.Series(lr_sarima.predict(X_test_s), index=y_test.index)
        
        results["SARIMA"] = {
            "MAE": mean_absolute_error(y_test, sarima_pred),
            "RMSE": np.sqrt(mean_squared_error(y_test, sarima_pred)),
            "MAPE": calculate_mape(y_test, sarima_pred)
        }
    ...
    metrics_comparison, all_model_predictions = evaluate_all_forecasting_models(train_df, test_df)
    
    comp_df = pd.DataFrame(metrics_comparison).T
    st.markdown("#### 🤖 Model Comparison Matrix")
    st.dataframe(comp_df.style.highlight_min(axis=0, color='#1E3A8A'), width='stretch')
    ...
    # Horizon prediction extension sequence dynamically matching slider step counts
    ...
    st.dataframe(forecast_display_df, width='stretch')
...
# ----------------------------------------------------
    ...
    st.dataframe(table[["Date", "Sales", "Anomaly Score"]], width='stretch')
...
# ----------------------------------------------------
    ...
    st.dataframe(table_segments[["Sub Category", "Cluster", "Average Sales", "Total Sales", "Orders"]], width='stretch')
